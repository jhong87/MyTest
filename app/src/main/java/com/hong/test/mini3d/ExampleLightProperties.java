...
import com.hong.min3d.objectPrimitives.Box;
import com.hong.min3d.objectPrimitives.Sphere;
import com.hong.min3d.vos.Light;
import com.hong.min3d.vos.LightType;
import com.hong.min3d.vos.Number3d;
...
public class ExampleLightProperties extends RendererActivity {
    ...
    protected void onCreateSetContentView() {
        setContentView(R.layout.mini3d_lightproperties_layout);
    ...
    OnSeekBarChangeListener seekChange = new OnSeekBarChangeListener() {
        public void onProgressChanged(SeekBar $s, int $progress, boolean $touch) {
            ...
            } else if (i == R.id.seekbarA2) {
                _light.position.setY(n * 3);

            } else if (i == R.id.seekbarA3) {
                _light.position.setZ(n * 3); // +3 + n*6);

            } else if (i == R.id.seekbarB1) {//_light.spotDirection.setX(n);
                scene.camera().position.x = n * 10;

            } else if (i == R.id.seekbarB2) {//_light.spotDirection.setY(n);
                scene.camera().position.y = n * 10;

            } else if (i == R.id.seekbarB3) {//_light.spotDirection.setZ(n);
                scene.camera().position.z = n * 10;

            } else if (i == R.id.seekbarC1) {
                n += 1; //[0,2]
                n *= 45 + 4;
                if (n > 90) n = 180;
